...
/* The core we inserted single-step breakpoint for. */
static int singlestep_core_num;
  ...
  if (execution_direction == EXEC_FORWARD
      ...
      && gdbarch_software_single_step (gdbarch, get_current_frame ()))
    {
	 ...
	 `wait_for_inferior'.  */
      ...
      singlestep_core_num = target_get_core_number ();
      singlestep_pc = pc;
    }
  return hw_step;
}
  ...
  if (singlestep_breakpoints_inserted_p)
    {
	 ...
	 past it.  */
      if ((!ptid_equal (ecs->ptid, singlestep_ptid)
	   || ecs->core_number != singlestep_core_num)
	  && ecs->event_thread->suspend.stop_signal == GDB_SIGNAL_TRAP)
	{
	  struct regcache *regcache;
	  struct address_space *aspace;
	  ...
	  if (ecs->core_number != singlestep_core_num)
	    target_set_core_number (singlestep_core_num);
	  ...
	  if (single_step_breakpoint_inserted_here_p (aspace, pc))
		...
		{
		  fprintf_unfiltered (gdb_stdlog,
				      "infrun: [%s (core %d)] hit step over single-step"
				      " breakpoint of [%s (core %d)]\n",
				      target_pid_to_str (ecs->ptid), ecs->core_number,
				      target_pid_to_str (singlestep_ptid), singlestep_core_num);
		}
