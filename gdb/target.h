...
struct target_ops
  {
    ...
    int (*to_insert_mc_hw_breakpoint) (struct target_ops *, struct gdbarch *, struct bp_target_info *, int)
      TARGET_DEFAULT_RETURN (-1);
    int (*to_remove_mc_hw_breakpoint) (struct target_ops *, struct gdbarch *, struct bp_target_info *, int)
      TARGET_DEFAULT_RETURN (-1);
    int (*to_multicore_hw_breakpoint) (struct target_ops *)
      TARGET_DEFAULT_RETURN (-1);
    int (*to_multicore_hw_watchpoint) (struct target_ops *)
      TARGET_DEFAULT_RETURN (-1);
    int (*to_get_core_number) (struct target_ops *)
      TARGET_DEFAULT_RETURN (-1);
    void (*to_set_core_number) (struct target_ops *, int)
      TARGET_DEFAULT_IGNORE ();
    ...
    int (*to_remove_mask_watchpoint) (struct target_ops *,
				      CORE_ADDR, CORE_ADDR, int)
      TARGET_DEFAULT_RETURN (1);
    int (*to_remove_mc_watchpoint) (struct target_ops *, CORE_ADDR, int, int, struct expression *, int)
      TARGET_DEFAULT_RETURN (-1);
    int (*to_insert_mc_watchpoint) (struct target_ops *, CORE_ADDR, int, int, struct expression *, int);
      TARGET_DEFAULT_RETURN (-1);
...
#define target_multicore_hw_breakpoint() \
     (*current_target.to_multicore_hw_breakpoint) (&current_target)
#endif

#ifndef target_multicore_hw_watchpoint
#define target_multicore_hw_watchpoint() \
     (*current_target.to_multicore_hw_watchpoint) (&current_target)
#endif

#ifndef target_get_core_number 
#define target_get_core_number() \
     (*current_target.to_get_core_number) (&current_target)
#endif

#ifndef target_set_core_number 
#define target_set_core_number(core) \
     (*current_target.to_set_core_number) (&current_target, core)
#endif

#ifndef target_insert_mc_hw_breakpoint
#define target_insert_mc_hw_breakpoint(gdbarch, bp_tgt, core_number) 		     \
     ((*current_target.to_multicore_hw_breakpoint)(&current_target)  		    	     \
	? (*current_target.to_insert_mc_hw_breakpoint) (&current_target, gdbarch, bp_tgt, core_number) \
        : (*current_target.to_insert_hw_breakpoint) (&current_target, gdbarch, bp_tgt))

#define target_remove_mc_hw_breakpoint(gdbarch, bp_tgt, core_number) 		     \
     ((*current_target.to_multicore_hw_breakpoint) (&current_target)			     \
        ? (*current_target.to_remove_mc_hw_breakpoint) (&current_target, gdbarch, bp_tgt, core_number) \
        : (*current_target.to_remove_hw_breakpoint) (&current_target, gdbarch, bp_tgt))
#endif

#ifndef target_insert_mc_watchpoint
#define target_insert_mc_watchpoint(addr, len, type, cond, core_number)	   \
     ((*current_target.to_multicore_hw_watchpoint)(&current_target)			   \
       ? (*current_target.to_insert_mc_watchpoint) (&current_target, addr, len, type, cond,	   \
						    core_number)	   \
       : (*current_target.to_insert_watchpoint) (&current_target, addr, len, type, cond))
#endif

#ifndef target_remove_mc_watchpoint
#define target_remove_mc_watchpoint(addr, len, type, cond, core_number)	   \
     ((*current_target.to_multicore_hw_watchpoint)(&current_target)			   \
       ? (*current_target.to_remove_mc_watchpoint) (&current_target, addr, len, type, cond, \
						    core_number)	   \
       : (*current_target.to_remove_watchpoint) (&current_target, addr, len, type, cond))
#endif
